...
class LocalRuleReviewer(Reviewer):
    ...
    RULES 类属性 
    (
    rule_id,        // 规则编号,下游修复器/评测按它分派
    severity,       // 命中时的严重级(规则人为预设)
    pattern,        // 编译好的正则,判定"是否命中"
    title,          // 给人类看的问题标题
    explanation,    // 解释
    fix,            // 修复建议与回归测试建议,直接写进 Finding
    test            // 回归测试建议,直接写进 Finding
    )           
    """
    RULES = [
        (
...
class OpenAICompatibleReviewer(Reviewer):
    ...
    # 用 urllib 发 HTTP;把 HTTP 错误、网络错误、JSON 解析错误统一转成带 provider 名字的 RuntimeError
    def _request_json(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not isinstance(result, dict):  # 不是 dict 就报错,避免下游 KeyError
            raise RuntimeError("%s returned a non-object JSON response" % self.provider)
        return result
    # 响应净化:(path, line) 不在新增行集合就丢弃;非法 severity 兜底 medium;所有文本字段截断
    @staticmethod
...
# 组合审查器,把多个 Reviewer 的结果合并,去重,按严重级别排序
