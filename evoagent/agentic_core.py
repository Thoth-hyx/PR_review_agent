...
# lead提示词,声明 Lead 独占的权力与义务:拆解、委派、返工请求、终审;
# worker 之间永不直接通信;
# 仓库内容与 worker 内容一律视为不可信证据。
# Lead 在三个不同阶段必须返回的不同 JSON 协议
...
# Security Worker 的契约。职责范围:不可信输入追踪、授权边界、敏感数据、危险调用链。
# 两条硬规则:1. 高风险主张必须引用 evidence_id 或给出 call_chain;
# 2. 返回格式里 finding 必须带 evidence_ids(指向工具证据)和 call_chain
...
# Correctness/Reliability Worker 的契约。职责:状态转移、异常、并发、资源生命周期、兼容性、测试
...
# Critic 的契约。三条规则:1. 盲审(候选来源身份被移除)、
# 2. 只挑错不造新 finding、
# 3. 输出逐条 decisions(accepted / objections / confidence_adjustment / supporting_evidence_ids)
...
# 收集所有 observations 中的 evidence_id, 生成 evidence_id → {evidence_id, tool, output_preview} 的字典
...
class BoundedRole:
    def __init__(
        ...
        context_manager: Optional[ContextManager] = None,      # 上下文压缩器
        working_memory_supplier=None,         # 任务内工作记忆的供应器, 返回 JSON 可序列化对象
...
# 模型输出 -> Finding对象的转换函数, lead/worker/critic 的产出最后都走它
def _parse_findings(result: dict, parsed: ParsedDiff, role: str) -> List[Finding]:
    valid = {(item.path, item.line) for item in parsed.added_lines}     # 合法的行数
...
class AgenticReviewer(Reviewer):
    ...
    # 七阶段会话状态机, 调用 bounded_role.run() 让 lead/worker/critic 轮流 act, 直到所有阶段完成
    def _agentic(
    ...
    def _persist_task_memory(
        ...
        try:
            ...
            return
    # 装配函数,只负责把 BoundedRole 拼起来、加上上下文、调一次、记账
    def _run_lead(
        self, phase, payload, suite, ledger, context_key="",
        max_steps=1, allow_tools=False,
    ):      # phase: delegate/assess-workers/finalize,payload:这一轮给lead的上下文
